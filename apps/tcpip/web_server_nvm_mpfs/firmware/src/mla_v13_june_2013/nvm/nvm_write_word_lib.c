...
//void NVMLoadBuffer(DWORD *addr, DWORD buffer[], BYTE *ptr, WORD size)
//{
//        memset(&buffer, 0xffffffff, 4096);      
//        memcpy(buffer, addr, 4096); //overwrite/save backup old data
//        memcpy(buffer, ptr, size); //copy new value from httpNetData into buffer
//}
void NVMUpdate(DWORD *addr, DWORD buffer[])
{
        WORD i;
        NVMErasePage(addr);   //must perform erase
        i = 0;
